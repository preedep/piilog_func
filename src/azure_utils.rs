...
pub async fn get_azure_access_token(
    ...
    match access_token {
        None => {
            ...
            credential.get_token(TOKEN_ENDPOINT).await
        }
        Some(token) => {
...
pub async fn get_certificate_from_key_vault(
    account_name: &str,
    key_name: &str,
) -> azure_core::Result<KeyVaultGetSecretResponse> {
    ...
    let res = match client_res {
        Ok(client) => client.get(key_name).await,
        Err(e) => {
